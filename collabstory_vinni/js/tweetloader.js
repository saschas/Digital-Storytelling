...
(function() {
  ...
  tooltip = function(tweet) {
    console.clear(); 
    console.log(buildStory(tweet.id));
  ...
  var cleanTweetText = function(text) {
    var mentionsRegex = /((@[a-z|_]* ?)*)*/i;
    var linebreaksRegex = /(\r\n|\n|\r)/gm;
    var hashtagRegex = /#[a-z|-]*/ig;
    return text.replace(mentionsRegex, '').replace(linebreaksRegex, '').replace(hashtagRegex, '').trim();
  };

  var buildStory = function(startId) {
    if (!startId || startId === null || startId === 'null') return '';
    ...
    return (buildStory(tweet.parent) + ' ' + cleanTweetText(tweet.text)).trim();
  };


  document.body.addEventListener('doneLoadingTweets', function (e) { 
    //console.log(buildStory("605374015381905408"));
  }, false);
